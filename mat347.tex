...
\begin{definition}[Integral Domain]
    An \bluebf{integral domain} is a commutative ring with unit, and no zero divisors. i.e. $\forall a,b\in R,~~ab=0\implies a=0\lor b=0.$
\end{definition}
\begin{proposition}
    \bluebf{Cancellation law:} in an integral domain, if $ab=ac$ where $a\neq0,$ then $b=c.$
    \begin{proof}
...
\begin{definition}[Subring]
    If $R$ is a ring, a subset $S\subseteq R$ is a \bluebf{subring} if the restriction to $S$ of the ring operation from $R$ make $S$ into a ring. i.e.
    \begin{itemize}
        \item $0\in S$
...
\textbf{FACT:} $\Z[\omega]$ is the set of all elements of $\Q[\sqrt D]$ whose norms are in $\Z,$ i.e. $\Z[\omega]=N^{-1}(\Z).$ It is always true that $\Z[D]\subseteq N^{-1}(\Z),$ but they are not equal if $D\equiv1\pmod4.$ The proof is a \bluebf{challenge}.
...
\begin{definition}[Ring Homomorphism]
    A \bluebf{ring homomorphism} $\varphi:R\to S$ is a map that respects addition and multiplication. That is, if $a,b\in R$ then \begin{align}
...
we require this to be equal to $rr'+N.$ Hence, we require $nr'-rn'\in N$ which is true if $nr'$ and $rn'$ are both in $N.$ So, the multiplication on $R/N$ is well defined if $RN\subseteq N$ and $NR\subseteq N$ \begin{definition}[Ideal]
    A subring $N\subseteq R$ is an \bluebf{ideal} if $RN\subseteq N$ and $NR\subseteq N.$ In this case, $R/N$ is a ring with the multiplication defined above.
\end{definition}
\subsection{Ideals}
...
\begin{definition}[Ideal]a
    In a ring $R,$ a subring $I$ is \begin{itemize}
        \item a \bluebf{left ideal} if $\forall r\in R,~\forall i\in I,$ $ri\in I.$ 
        \item a \bluebf{right ideal} if $\forall r\in R,~\forall i\in I,$ $ir\in I.$
        \item a \bluebf{(two-sided) ideal} if it is both a left and right ideal.
    \end{itemize}
\end{definition}
...
\end{theorem}
If $A\subseteq R,$ then we can define the \bluebf{ideal generated by $A$} denoted $(A).$ If $A$ is a singleton $\{a\},$ we denote $(a),$ and if $A$ is a finite set $\{a_1,\ldots,a_n\},$ we denote $(a_1,\ldots,a_n).$ \begin{align}
...
\begin{definition}[Maximal Ideal]
    An ideal $M$ is a \bluebf{maximal ideal} if there is no \textbf{proper} ideal $I$ s.t. $M\subset I\subset R$ and $M\neq I.$
\end{definition}
