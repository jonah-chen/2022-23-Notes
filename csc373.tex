...
\begin{itemize}
    ...
    \item The \bluebf{traveling salesman problem}(TSP) is: given an weighted undirected complete graph $G=(V,E),$ find a tour of $G$ with minimum total weight. A tour is a cycle that visits every vertex exactly once.
...
\begin{itemize}
    ...
    \item \begin{algorithmic}[1]
        \Procedure{GreedyVertexCover}{$G$}
        \State $S\gets\emptyset$
        \State $E'\gets E$
        \State $M\gets\emptyset$\Comment{Matching (only for purposes of proof)}
        \While{$E'\neq\emptyset$}
...
\begin{itemize}
    ...
    \item The greedy algorithm does not work. We will use a common approximation scheme called \bluebf{ILP Relaxation}.
...
\begin{itemize}
    ...
    \item As we did not sort the jobs, this is an \bluebf{online algorithm}.
