...
\begin{itemize}
    \item Combining the diffusion approximation with the neutron balance equation, we obtain the \bluebf{neutron diffusion equation} \begin{equation}
        (\mathrm{div}(D\nabla)+\Sigma_a)\Phi=Q
    \end{equation}
    ...
    \end{align}
    With neutron transport theory, we get a better approximation for $\delta_{ex}=0.71\lambda_{tr}^A.$ This value $\delta_{ex}$ is called the \bluebf{extrapolation length}.
\end{itemize}
...
\begin{itemize}
    ...
    \end{align}
    We will define the \bluebf{diffusion area} as \begin{align}
